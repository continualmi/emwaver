...
use coremidi_sys::{
    Byte, MIDIPacket, MIDIPacketList, MIDIPacketListAdd, MIDIPacketListInit, MIDIPacketNext,
...
impl<'a> Iterator for PacketListIterator<'a> {
    ...
    fn next(&mut self) -> Option<&'a Packet> {
        if self.count > 0 {
            // On some platforms, CoreMIDI packet lists may not preserve 4-byte alignment between
            // consecutive `MIDIPacket` records. Creating a Rust reference to an unaligned
            // `MIDIPacket` is UB and will panic in debug builds.
            //
            // Treat the memory as a packed `Packet` and perform unaligned reads inside the
            // accessor methods.
...
///
/// Important: `MIDIPacket` values inside a packet list are not guaranteed to be 4-byte aligned
/// on all targets. Use a packed representation and read fields with `read_unaligned`.
#[repr(C, packed)]
pub struct Packet {
...
impl Packet {
    ...
    pub fn timestamp(&self) -> Timestamp {
        let ptr = std::ptr::addr_of!(self.time_stamp) as *const MIDITimeStamp;
        unsafe { ptr.read_unaligned() as Timestamp }
    }
    ...
    /// ```
    pub fn data(&self) -> &[u8] {
        let len_ptr = std::ptr::addr_of!(self.length) as *const u16;
        let data_len = unsafe { len_ptr.read_unaligned() as usize };
