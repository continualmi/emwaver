...
const STATE_DIR_NAME = ".emwaver";
const STATE_FILE_NAME = "wavelets-tabs.json";
...
function stateDir(rootDir: string): string {
  return `${normalizeRoot(rootDir)}/${STATE_DIR_NAME}`;
}

function stateFile(rootDir: string): string {
  return `${stateDir(rootDir)}/${STATE_FILE_NAME}`;
}
...
export async function readWaveletsTabState(rootDir: string | null): Promise<WorkspaceTabState | null> {
  if (!rootDir) {
    return null;
  }
...
export async function writeWaveletsTabState(
  rootDir: string | null,
  openFiles: string[],
  active: string | null,
): Promise<void> {
