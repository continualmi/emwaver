...
async function cc1101ReadReg(addr) {
    ...
    const response = parseRawPayload(await SPI.transfer([cmd, 0x00], { cs: CC1101_CS, rxLength: 2 }));
...
async function cc1101WriteBurst(addr, data) {
    ...
    const response = parseRawPayload(await SPI.transfer(tx, { cs: CC1101_CS }));
