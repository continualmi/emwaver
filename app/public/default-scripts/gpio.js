...
function gpioRead() {
    ...
    if (selectedMode === "in") {
        emw.send("gpio in --pin=" + selectedPin);
    }

    let response = emw.send("gpio read --pin=" + selectedPin);
...
function gpioWrite(value) {
    ...
    let outResp = emw.send("gpio out --pin=" + selectedPin);
    let writeResp = emw.send(value ? "gpio high --pin=" + selectedPin : "gpio low --pin=" + selectedPin);
    let readResp = emw.send("gpio read --pin=" + selectedPin);
