...
- The setup surface can query the ESP32 binary Wi-Fi status opcode over USB/BLE and reports provisioned, station online/offline, current station IP when firmware provides it, reconnecting, last station disconnect reason, and authenticated state for local diagnostics.
