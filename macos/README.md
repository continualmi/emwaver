...
- When UID connection checks are enabled, the app queries `EMW_OP_HARDWARE_UID_GET` after USB/BLE connection and uses that local hardware UID to merge the same physical device across transports when known. Wi-Fi uses fresh UID probe responses as its live connection metric, not a cached UID. This UID is only for local labels/diagnostics/device-list deduplication; it must not be used for account activation, ownership, device limits, or subscription checks.
