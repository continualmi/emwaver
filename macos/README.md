...
- Wi-Fi device presence is confirmed by the hardware UID command over the WebSocket while UID connection checks are enabled. mDNS/manual records are only addresses to try; the local device list only shows a Wi-Fi endpoint after a fresh valid 6-byte UID response, and UID liveness failure removes it. Users can disable automatic UID connection checks from the device sheet, which stops UID liveness/validation probes and leaves connection state to the active transport.
...
- The local device selector is the preferred transport for the next script run only. Discovery and UID checks continue across transports; automatic script-target selection ranks USB, then BLE, then Wi-Fi unless the user explicitly selects a transport.
...
- When UID connection checks are enabled, the app queries `EMW_OP_HARDWARE_UID_GET` after USB/BLE connection, refreshes connected USB/BLE UID checks during the regular connection poll, and uses that local hardware UID to merge the same physical device across transports when known. Wi-Fi uses fresh UID probe responses as its live connection metric, not a cached UID. This UID is only for local labels/diagnostics/device-list deduplication; it must not be used for account activation, ownership, device limits, or subscription checks.
