...
| Sampler/retransmit lanes can use Wi-Fi without corrupting command responses | `sampler.c`, `wifi_transport_send_stream_lane`, `wifi_transport_send_buffer_status`; macOS/daemon `BS` filtering tests | `[x]` |
...
| macOS compile validation passes | `xcodebuild build-for-testing -project macos/EMWaver/EMWaver.xcodeproj -scheme EMWaver -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO` was run after the latest macOS manual port validation change | `[x]` |
...
The implementation is not complete. Code and documentation coverage are in place for the macOS, daemon/gateway, and ESP32 firmware slices, but the objective still requires real ESP32-S3 hardware validation for same-LAN script execution and VPN-by-IP execution.
