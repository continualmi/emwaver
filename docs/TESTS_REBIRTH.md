...
| Linux validation runbook | `hosted pass / hardware pending` | `scripts/rebirth-linux-validation.sh` records Linux ALSA/USB and Bluetooth diagnostics, calls the generic hardware validation helper, always runs gateway-daemon simulator fallback, and can run hardware-backed `emwaver gateway --daemon-fallback` render/event validation. Hosted Ubuntu passed in Rebirth Platform Validation run `25429754048`; real USB/BLE hardware still needs a Linux host with a visible board. |
