...
| Daemon CI workflow | `pass` | GitHub Actions run `25429739081` passed on main: builds `emwaver`, tests `emwaver-runtime`/`emwaver-device`, verifies direct UI-only and simulator-backed runtime, systemd unit generation, gateway-to-daemon simulator render/event flow, CLI `gateway --daemon-fallback --sim-device`, and packaged install smoke. |
| CLI Gateway Release workflow | `pass` | GitHub Actions run `25430201233` passed on main: built `EMWaver-linux-x64.tar.gz` and `EMWaver-macos-cli.tar.gz`, smoke-tested packaged gateway `/health`, and verified packaged `emwaver gateway --daemon-fallback --sim-device` script rendering plus UI event dispatch. Artifacts were published to the `emwaver-preview` release. |
...
| Windows simulator adapter | `source added / build blocked` | `Scripting/SimulatorCommandBridge.cs` reads the shared fixture JSON and provides a `sendPacket` delegate compatible with `ScriptEngine.Setup`; local build remains blocked here because `dotnet` is unavailable. |
| Windows simulator engine tests | `pass` | `windows/EMWaver.Tests` runs a hardware-touching `.emw` script through `ScriptEngine` and `SimulatorCommandBridge`, asserting UI output from `simulator/fixtures/basic-board.json`; hosted Windows CI passed in Rebirth Platform Validation run `25429754048`. |
...
| Virtual transport simulator decision | `doc pass` | `simulator/VIRTUAL_TRANSPORT.md` keeps fake MIDI/USB out of the portable baseline, documents macOS/Linux/Windows/iOS/Android feasibility, and requires any future virtual transport to reuse shared simulator fixtures. |
...
| Linux validation runbook | `hosted pass / hardware pending` | `scripts/rebirth-linux-validation.sh` records Linux ALSA/USB and Bluetooth diagnostics, calls the generic hardware validation helper, always runs gateway-daemon simulator fallback, verifies systemd user unit generation, can opt into a real systemd user-service render smoke with `EMWAVER_VALIDATE_SYSTEMD=1`, and can run hardware-backed `emwaver gateway --daemon-fallback` render/event validation. Hosted Ubuntu passed in Rebirth Platform Validation run `25429754048`; real USB/BLE hardware still needs a Linux host with a visible board. |
| Gateway daemon simulator | `done` | `scripts/rebirth-gateway-daemon-sim-validation.sh` starts the built gateway and Rust daemon with `--sim-device`, then drives `/v1/ws` as a browser client to verify script render and UI event dispatch. It also runs in fallback mode for `emwaver gateway --daemon-fallback --sim-device`. This runs in hosted Ubuntu Daemon CI. |
| Install smoke | `done` | `scripts/rebirth-install-smoke.sh` installs into a temporary prefix, verifies `$prefix/bin/emwaver`, verifies packaged gateway assets under `$prefix/share/emwaver/gateway`, verifies default scripts under `$prefix/share/emwaver/assets/default-scripts`, starts the installed gateway health endpoint, then starts installed `emwaver gateway --daemon-fallback --sim-device` and verifies daemon-backed script render plus UI event dispatch through `scripts/verify-gateway-daemon-render.mjs`. This runs in hosted Ubuntu Daemon CI. |
| Hosted Linux/Windows validation workflow | `pass` | `.github/workflows/rebirth-platform-validation.yml` run `25429754048` passed hosted Ubuntu Linux validation and hosted Windows restore/build plus simulator tests. Hardware-in-the-loop jobs remain dispatch-only on self-hosted runners labeled `emwaver-hardware`. |
