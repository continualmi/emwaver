...
| Gateway daemon simulator | `done` | `scripts/rebirth-gateway-daemon-sim-validation.sh` starts the built gateway and Rust daemon with `--sim-device`, then drives `/v1/ws` as a browser client to verify script render and UI event dispatch. This runs in hosted Ubuntu Daemon CI. |
