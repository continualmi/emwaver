...
| Hosted Linux/Windows validation workflow | `pass` | `.github/workflows/rebirth-platform-validation.yml` run `25430732056` passed hosted Ubuntu Linux validation and hosted Windows restore/build plus simulator tests. Hardware-in-the-loop jobs remain dispatch-only on self-hosted runners labeled `emwaver-hardware`. |
