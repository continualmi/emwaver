...
| Linux validation runbook | `added / blocked` | `scripts/rebirth-linux-validation.sh` records Linux ALSA/USB and Bluetooth diagnostics and calls the generic hardware validation helper. USB hardware uses `EMWAVER_DEVICE_ID`; BLE hardware uses `EMWAVER_HARDWARE_TRANSPORT=ble`. It degrades cleanly on macOS but must be rerun on Linux with a visible board. |
