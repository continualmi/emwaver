...
| Windows simulator engine tests | `added / pending CI` | `windows/EMWaver.Tests` runs a hardware-touching `.emw` script through `ScriptEngine` and `SimulatorCommandBridge`, asserting UI output from `simulator/fixtures/basic-board.json`; hosted Windows CI runs it through `scripts/rebirth-windows-validation.ps1 -Ci`. |
...
| Linux validation runbook | `added / blocked` | `scripts/rebirth-linux-validation.sh` records Linux ALSA/USB and Bluetooth diagnostics, calls the generic hardware validation helper, always runs gateway-daemon simulator fallback, and can run hardware-backed `emwaver gateway --daemon-fallback` render/event validation. USB hardware uses `EMWAVER_DEVICE_ID`; BLE hardware uses `EMWAVER_HARDWARE_TRANSPORT=ble`. It degrades cleanly on macOS but must be rerun on Linux with a visible board. |
