...
| Install smoke | `done` | `scripts/rebirth-install-smoke.sh` installs into a temporary prefix, verifies `$prefix/bin/emwaver`, verifies packaged gateway assets under `$prefix/share/emwaver/gateway`, and starts the installed gateway health endpoint. This runs in hosted Ubuntu Daemon CI. |
