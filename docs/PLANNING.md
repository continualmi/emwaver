...
| `P0` | UI snapshot-only runtime | `Gateway slice complete / native reversed` | Gateway-owned session list/snapshot/event/stop CLI workflows are implemented; native Agent snapshot/event tools replaced by hardware primitive tools (`spi_transfer`, `gpio_read`, `gpio_write`, etc.). See `AGENT_EVAL_RUNTIME.md`. |
