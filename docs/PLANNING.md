...
| `P0` | UI snapshot-only runtime | `planned` | `UI_SNAPSHOT_RUNTIME_MIGRATION.md` removes script-visible `console.*`, `_scriptLog`, and `script.log`; CLI/Agent automation should use session list, UI snapshots, UI events, and stop/reset commands. |
