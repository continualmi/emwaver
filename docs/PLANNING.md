...
- Agent-driven hardware automation: make the local CLI/Gateway loop good enough that a coding agent can write custom `.emw` scripts, run them against real hardware, inspect UI snapshots/logs, send UI events, stop/reset scripts, and iterate quickly from the terminal.
...
4. Design the multi-device runtime contract for two simultaneous boards: device ids, transport labels, per-device status, routing commands to a chosen board, and UI/snapshot attribution.
5. Review older/generated hardware catalog IDs and decide whether they map to imported repos or need separate cleanup.
6. Inventory public web and hardware folders for duplicated board/module images and choose canonical `hardware/<repo-name>/` asset paths.
