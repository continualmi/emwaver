...
- Add streaming support for sampler/retransmit status without blocking command responses. ESP32 firmware can now route sampler stream lanes over the Wi-Fi WebSocket for sessions that start sampling over Wi-Fi, ingest Wi-Fi retransmit stream lanes into the shared circular RX buffer, and send Wi-Fi `BS` buffer-status frames using sequence `0`; macOS and daemon command sequences start at `1`, skip reserved sequence `0` after wraparound, ESP32 rejects envelope command frames with reserved sequence `0`, Wi-Fi retransmit uses `BS` frames for host-side pacing, and the daemon ignores those sequence-`0` `BS` frames instead of appending them to captured script data.
