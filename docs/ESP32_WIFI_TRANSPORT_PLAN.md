...
- Add streaming support for sampler/retransmit status without blocking command responses. ESP32 firmware can now route sampler stream lanes over the Wi-Fi WebSocket for sessions that start sampling over Wi-Fi, using sequence `0` so those stream frames do not complete pending macOS command waits; macOS command sequences start at `1`.
