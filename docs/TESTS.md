...
- Expected: CLI and macOS app can run scripts through Wi-Fi without accounts, cloud relay, hosted activation, or Wi-Fi secrets; second simultaneous clients are rejected as busy; Wi-Fi drops leave the runtime recoverable; USB/BLE provisioning remains available.
