...
| Hosted Linux/Windows validation workflow | `added / pending CI` | `.github/workflows/rebirth-platform-validation.yml` runs hosted Ubuntu Linux dry-run validation and hosted Windows restore/build plus simulator tests. Hardware-in-the-loop jobs are dispatch-only on self-hosted runners labeled `emwaver-hardware`. |
