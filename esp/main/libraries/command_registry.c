                           ...
                           const command_arg_value_t *values)
{
    ...
    if (types_match(entry, (cmd_arg_type_t[]){CMD_ARG_STRING, CMD_ARG_STRING, CMD_ARG_STRING, CMD_ARG_STRING}, 4)) {
        CALL_HANDLER(entry, void (*)(const char *, const char *, const char *, const char *),
                     values[0].str_val, values[1].str_val, values[2].str_val, values[3].str_val);
        return;
    }
