...
- Initial Wi-Fi provisioning is available through the shared binary command lane (`EMW_OP_WIFI_CONFIG`) so USB MIDI and BLE can chunk SSID/password/hostname/local pairing secret into NVS. The same binary status command reports provisioned, authenticated, station online/offline, reconnecting state, and the last ESP-IDF station disconnect reason; the older text command path also accepts `wifi provision --ssid=<ssid> --password=<password> --secret=<local-secret> [--hostname=<emwaver-name>]`, plus `wifi status` and `wifi clear` for diagnostics/recovery.
