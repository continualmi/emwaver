...
- A Wi-Fi WebSocket client must complete challenge/auth within the firmware auth timeout; otherwise firmware sends `auth timeout` and closes the pending unauthenticated socket so it cannot reserve the active session slot indefinitely. Firmware also closes the socket immediately after an `auth fail` response so a rejected local client cannot linger after the active slot is cleared.
