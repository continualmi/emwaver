...
- Sampler streams now route to the transport that started sampling. Wi-Fi-started sampler sessions send stream lanes over the authenticated WebSocket with reserved envelope sequence `0`, leaving echoed command-response sequence ids available for request/response matching.
- Wi-Fi retransmit stream lanes now fill the same firmware circular RX buffer as USB retransmit data and return Wi-Fi `BS` buffer-status frames over the WebSocket with reserved envelope sequence `0`; incoming sequence-`0` envelopes are accepted only when they are stream-only frames, so command-bearing envelopes still require a nonzero request sequence.
- Wi-Fi transport foundation is active behind the `CONFIG_EMWAVER_ENABLE_WIFI_TRANSPORT` / `EMWAVER_ENABLE_WIFI_TRANSPORT` compile-time feature gate, enabled by default in `main/Kconfig.projbuild`: provisioned ESP boards join station-mode Wi-Fi from NVS, advertise `_emwaver._tcp` on port `3922` with target-aware instance names plus board/version/capability/local-id TXT records, and expose an authenticated WebSocket at `/v1/ws` carrying the same EMWaver SysEx/superframe bytes as USB MIDI and BLE-capable targets.
- Wi-Fi discovery is published only after the authenticated WebSocket server and `/v1/ws` handler are ready, so mDNS does not advertise a local endpoint that cannot accept control sessions. If mDNS setup fails, firmware keeps the WebSocket available for manual IP/hostname connections and logs the discovery failure.
...
- Initial Wi-Fi provisioning is available through the shared binary command lane (`EMW_OP_WIFI_CONFIG`) so USB MIDI and BLE can chunk SSID/password/hostname/local pairing secret into NVS. The binary lane requires `EMW_WIFI_CFG_BEGIN` before staged field/apply/pairing-reset operations and clears staged credentials after successful apply/reset or clear so stale local secrets are not reused. The same binary status command reports provisioned, authenticated, station online/offline, reconnecting state, the last ESP-IDF station disconnect reason, the current station IPv4 address when online, and whether sampler/transmit runtime work is active; the binary pairing-reset command can rotate the local pairing secret without replacing SSID/password/hostname. The older text command path also accepts `wifi provision --ssid=<ssid> --password=<password> --secret=<local-secret> [--hostname=<emwaver-name>]`, plus `wifi pair --secret=<local-secret>`, `wifi status`, and `wifi clear` for diagnostics/recovery; text `wifi status` mirrors the reconnecting state, runtime-active state, and last disconnect reason and includes the advertised hostname and station IP for local USB/BLE debugging.
...
- Provisioned Wi-Fi hostnames must fit the ESP32 mDNS hostname contract: up to 32 letters, numbers, or hyphens, without a leading or trailing hyphen. Invalid stored hostnames fall back to the generated `emwaver-xxxx` name on load.
...
- Resetting Wi-Fi pairing over the binary command lane or `wifi pair` updates only the local pairing secret in NVS and closes the active WebSocket session; existing SSID/password/hostname settings remain provisioned.
...
- The auth message must identify itself as `type=auth` with `protocolVersion=1`, and firmware checks the HMAC response with a constant-time comparison before accepting a Wi-Fi command session. Clients can opt into binary envelope version `1` during auth; otherwise firmware keeps accepting raw 48-byte SysEx WebSocket frames for compatibility.
- Wi-Fi binary envelope version `1` uses `EMW`, protocol version, frame kind, sequence id, payload length, and payload bytes. The first supported frame kind carries the existing 48-byte EMWaver SysEx packet, so the firmware runtime command model stays shared with USB MIDI and BLE. For enveloped command frames, firmware echoes the request sequence id on the binary response frame and reserves sequence `0` for uncorrelated stream/status frames.
- Raw 48-byte SysEx WebSocket frames are accepted only for authenticated clients that did not opt into envelope version `1`, keeping envelope sessions sequence-correlated.
- A Wi-Fi WebSocket client must complete challenge/auth within the firmware auth timeout; otherwise firmware sends `auth timeout` and closes the pending unauthenticated socket so it cannot reserve the active session slot indefinitely. Firmware also closes the socket immediately after auth-timeout task creation failure, a failed challenge send, or an `auth fail` response so rejected or failed-bootstrap local clients cannot linger after the active slot is cleared.
...
- OTA SoftAP remains separate from the station-mode Wi-Fi runtime socket; starting OTA SoftAP explicitly suspends the authenticated runtime WebSocket, mDNS advertisement, active Wi-Fi session state, and station reconnect state before switching the radio into AP mode. If OTA SoftAP is stopped or fails to finish starting before a firmware reboot, provisioned station-mode runtime resumes without requiring manual reprovisioning.
