...
- Wi-Fi discovery is torn down if the authenticated WebSocket server or `/v1/ws` handler cannot start, so mDNS does not advertise a local endpoint that cannot accept control sessions.
