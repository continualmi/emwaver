...
- Initial Wi-Fi provisioning is available through the shared binary command lane (`EMW_OP_WIFI_CONFIG`) so USB MIDI and BLE can chunk SSID/password/hostname/local pairing secret into NVS. The older text command path also accepts `wifi provision --ssid=<ssid> --password=<password> --secret=<local-secret> [--hostname=<emwaver-name>]`, plus `wifi status` and `wifi clear` for diagnostics/recovery.
...
- The Wi-Fi WebSocket server keeps a single active authenticated socket and rejects another concurrent client as busy instead of letting a second app silently replace the active command owner.
