...
- Sampler streams now route to the transport that started sampling. Wi-Fi-started sampler sessions send stream lanes over the WebSocket with reserved envelope sequence `0`, leaving echoed command-response sequence ids available for request/response matching.
...
- Wi-Fi command clients use envelope version `1`; request/response traffic stays sequence-correlated.
