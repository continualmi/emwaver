...
- Wi-Fi binary envelope version `1` uses `EMW`, protocol version, frame kind, sequence id, payload length, and payload bytes. The first supported frame kind carries the existing 48-byte EMWaver SysEx packet, so the firmware runtime command model stays shared with USB MIDI and BLE.
