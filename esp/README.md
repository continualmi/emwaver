...
- Wi-Fi discovery is published only after the authenticated WebSocket server and `/v1/ws` handler are ready, so mDNS does not advertise a local endpoint that cannot accept control sessions.
