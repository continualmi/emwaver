...
- A Wi-Fi WebSocket client must complete challenge/auth within the firmware auth timeout; otherwise firmware sends `auth timeout` and closes the pending unauthenticated socket so it cannot reserve the active session slot indefinitely. Firmware also closes the socket immediately after a failed challenge send or an `auth fail` response so rejected or failed-bootstrap local clients cannot linger after the active slot is cleared.
