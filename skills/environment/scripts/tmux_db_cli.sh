...
SESSION_NAME="${WORKTREE_NAME}-db-cli"
...
if [[ ! -d "${DB_CLI_DIR}" ]]; then
  echo "db-cli project directory not found at '${DB_CLI_DIR}'" >&2
  exit 1
fi
...
tmux new-window -t "${SESSION_NAME}" -n "db-cli" -c "${DB_CLI_DIR}"
tmux send-keys -t "${SESSION_NAME}:db-cli.0" "cd '${DB_CLI_DIR}'" C-m
if [[ -f "${ENV_SCRIPT}" ]]; then
  tmux send-keys -t "${SESSION_NAME}:db-cli.0" "source '${ENV_SCRIPT}'" C-m
fi
tmux send-keys -t "${SESSION_NAME}:db-cli.0" "cargo check" C-m

tmux split-window -h -t "${SESSION_NAME}:db-cli" -c "${DB_CLI_DIR}"
tmux send-keys -t "${SESSION_NAME}:db-cli.1" "cd '${DB_CLI_DIR}'" C-m
tmux send-keys -t "${SESSION_NAME}:db-cli.1" "cargo test" C-m

tmux split-window -v -t "${SESSION_NAME}:db-cli.1" -c "${DB_CLI_DIR}"
tmux send-keys -t "${SESSION_NAME}:db-cli.2" "cd '${DB_CLI_DIR}'" C-m
tmux send-keys -t "${SESSION_NAME}:db-cli.2" "${EDITOR:-vim}" C-m
