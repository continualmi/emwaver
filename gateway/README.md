...
- Gateway discovers USB MIDI and Wi-Fi in one backend process. BLE discovery/control is available with `emw gateway serve --ble` or `emw gateway start --ble`; on macOS it is opt-in to avoid unstable background CoreBluetooth polling. Device listing is UID-backed: physical devices are exposed only after the backend can read a hardware UID.
