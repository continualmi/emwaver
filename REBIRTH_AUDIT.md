...
| Shared device simulator | `SIMULATOR.md`, `simulator/fixtures/basic-board.json`, `emwaver-runtime::SimulatorCommandBridge`, Apple `SimulatorScriptDevice`, `REBIRTH-045` through `REBIRTH-049` | shared fixture, Rust bridge, and Apple adapter added; Windows/Android adapters and optional virtual transport remain future work |
...
| Native remote-control scope | `REBIRTH.md`, `REBIRTH-050`, `LAUNCH_MVP.md`, macOS/Windows host socket code | same-machine localhost gateway control is now the core posture; macOS/Windows hosted `/v1/ws` fallback is gated behind `EMWAVER_HOSTED_REMOTE_CONTROL_ENABLED=1`; macOS Debug build passed, Windows build blocked by missing `dotnet`, and remaining hosted/session UI cleanup is still pending |
