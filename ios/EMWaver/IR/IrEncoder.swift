 ...
 * Swift port of the Android IrEncoder class.
 */
class IrEncoder {
    
    // MARK: - Protocol Definitions
    
    private static let protocolDefinitions: [String: String] = [
        "DAC4":
            "Frequency=38000\n"
            + "Zero=500,-1000\n"
            + "One=500,-500\n"
    ...
    // MARK: - RC6 Pattern Regex
    
    private static let rc6Pattern = try! NSRegularExpression(pattern: "RC6-(\\d+)-(\\d+)", options: [.caseInsensitive])
    ...
    /**
     ...
     - Parameters:
        - protocol: The name of the IR protocol (e.g., "NECx1", "RC5", "RC6-0-16")
        - device: The device code (D)
        - subdevice: The subdevice code (S). Use -1 if the protocol doesn't use a subdevice or uses default.
    ...
    static func encodeIR(protocol protocolName: String, device: Int, subdevice: Int, function: Int) -> [Double]? {
        ...
        baseDefinition = protocolDefinitions[baseProtocolName]
        ...
        // Add Device= and Function= lines (as done in the original code)
        if subdevice >= 0 {
            irpDefinitionBuilder += "Device=\(device).\(subdevice)\nFunction=\(function)\n"
        } else {
            irpDefinitionBuilder += "Device=\(device)\nFunction=\(function)\n"
        }
