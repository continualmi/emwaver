...
export default function OrderConfirmedPage() {
  ...
  async function doSignInAndClaim() {
    ...
    if (!auth) {
      setError(
        "Google sign-in is not configured yet (missing NEXT_PUBLIC_FIREBASE_* env)."
      );
      return;
    }
      ...
      <main className="mx-auto max-w-4xl px-5 py-10">
        <div className="rounded-3xl border border-[color:var(--line)] bg-[rgba(255,255,255,0.03)] p-6 md:p-10">
          ...
          <p className="pt-3 text-[15px] leading-7 text-[color:var(--ink-dim)]">
            If payment succeeded, you’ll receive a receipt from Stripe. You can optionally sign in to attach this purchase to your EMWaver account.
          </p>
          ...
          <div className="mt-6 space-y-3">
            ...
            {userEmail ? (
              <div className="text-xs text-[color:var(--ink-dim)]">Signed in as: {userEmail}</div>
            ) : null}
            ...
            <div className="pt-3 text-xs text-[color:var(--ink-dim)]">
              This is an early flow being built ahead of sales opening.
            </div>
          </div>
        </div>
      </main>
