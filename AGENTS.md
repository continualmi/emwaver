...
Wavelets Workspace also exposes a persistent “asset scripts” panel (collapsible) under the sidebar; asset scripts can be opened and previewed (via the top-bar `Preview` action) even without a project folder open. Wavelet project files can use `.emw` as a first-class extension (shown as an `EM` badge).
