...
- **Hardware:** **STM32 only** (one current-gen board) — intentionally minimal; the host provides everything else
...
- **Core UX:** **Script-centered** hardware exploration (script + UI together, instant execution, no reflashing) + **AI agents** that build and test autonomously
- **Surface area shipped:** Android app, iOS app, macOS app, Windows app
- **Built-in capabilities:** IR receiver + IR transmitter (plug-and-play hacking out of the box)
- **Expandable:** GPIO header for external modules (CC1101 sub-GHz, MFRC522 RFID, any SPI/I2C/UART sensor)
...
- **ELM**: the Agent UI + cloud-integrated workflows are **Pro-only**.
...
#### ELM (Electronics Language Models)

EMWaver’s in-house models are called **ELMs** (**E**lectronics **L**anguage **M**odels), versioned as:
- `ELM-1`, `ELM-2`, …
...
**EMWaver is the future of electronics development.**

Three reasons:

1. **Host-powered electronics** — EMWaver doesn't do anything by itself. It leverages a host (phone/laptop/RPi), extracting power, compute, UI, storage, and connectivity. You are never limited by the MCU.
2. **AI-first platform** — AI agents can write EMWaver code, generate UI, run scripts on real hardware, interact with the interfaces they created, and iterate — fully autonomously. Full chip exploits in minutes.
3. **Best learning platform for beginners** — plug in, open the app, start hacking. Zero toolchains, zero firmware flashing, zero prerequisites.
...
### Why it's different (updated vision)

- **Host-first electronics:** the EMWaver device intentionally does not "do everything" on the MCU. It connects to a host (phone/desktop/RPi) and leverages the host's **power, compute, storage, UI, and connectivity**. No on-device screen or buttons — the host screen *is* the interface. Unlimited storage on the host. Full internet connectivity through the host.
- **Cloud-native control graph:** hosts connect to EMWaver Cloud so **hosts can control other hosts** (and their attached EMWaver devices) from anywhere over the internet. A $35 RPi in your lab becomes remotely accessible from any device on Earth.
- **AI-first workflows:** the platform is designed so agents can do more than write code — they can **interact with the UIs generated by scripts** to test and iterate autonomously. We also train EMWaver-native **ELMs (Electronics Language Models)** — foundation models finetuned for hardware control.
- **Beginner-first learning:** plug in a device, open the app, and immediately start capturing and cloning IR signals. Start with built-in **IR receiver/transmitter**, then attach common modules (**CC1101** for sub-GHz, **MFRC522** for RFID) to learn by doing — without firmware toolchains, without installing anything beyond the app.
- **Best hacking multitool:** EMWaver directly plugs into a smartphone (no cables), has no screen/button limitations (massive host UI), unlimited host storage, cloud connectivity, AI agents, and is inexpensive compared to alternatives like Flipper Zero.

### EMWaver Scripts

EMWaver scripts are the essence of EMWaver:

- No compile, no flash — **instant execution** from the host
- Ultra-fast hardware exploration: change a line, see the result immediately
- In a single script you develop both:
  - low-level hardware interactions (SPI, I2C, UART, GPIO, ADC, PWM)
  - high-level user interfaces (buttons, sliders, plots, logs)
- Designed to answer one question: **how fast can you fully exploit all functionality of any chip/module/sensor?** Answer: lightning fast.
...
- On-device wireless workflows (connectivity comes from the host).
...
A very simple, powerful platform:

- One board, one firmware
- Apps on Android / iOS / macOS / Windows
- AI agents that build and test autonomously
- Cloud-connected hardware accessible from anywhere
- The best beginner experience in electronics
...
**EMWaver is the future of electronics development** — a host-powered, AI-first, beginner-friendly hardware exploration platform.

- Users explore hardware by writing/running **`.emw` scripts** that execute instantly on the host.
- A script defines both:
  - device/hardware interactions (SPI, I2C, UART, GPIO, ADC, PWM, IR), and
  - a rich UI that makes the script usable/repeatable.
- AI agents can autonomously write scripts, generate UIs, run them on real hardware, interact with the generated UI, and iterate.
...
- **Never read env secret files** (`.env`, `.env.*`, `secrets/*.env`, `secrets/**/*.env`) unless the user explicitly asks you to inspect them in this chat.
- You **may read documentation about env layout/policy** (for example `secrets/README.md`) without special permission.
