...
Full JavaScript runtime parity beyond the initial command/gpio/device API, local module loading, JSX/script-tree capture, and initial GTK script-tree rendering, the script UI event-handler invocation path, Linux hardware validation for BLE GATT I/O and ESP32 serial flashing, Wi-Fi provisioning UI/status, and packaged installers are staged behind the crate boundaries and are not complete yet.
