...
| `P0` | Agent interfaces | `partially migrated` | Repo-shipped production prompt files are removed; gateway/CLI and Apple shared iOS/macOS Agent paths now use API-key endpoint clients instead of EMWaver account chat routes. Remaining work: finish Windows/Android app clients and enrich native request context. |
