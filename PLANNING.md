...
| `P0` | Runtime + CLI | `in progress` | Rust runtime/device crates are extracted, `CommandBridge` decouples runtime from MIDI transport, selected-device daemon startup builds, and `emwaver run`/`doctor` are verified locally. Hardware-backed validation remains. |
...
4. Decide whether `emwaver run --device <id>` should add a direct headless runtime mode or stay a gateway/native-app controller.
