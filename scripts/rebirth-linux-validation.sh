...
echo
echo "== Gateway simulator validation =="
EMWAVER_GATEWAY_PORT="$GATEWAY_SIM_PORT" "$ROOT/scripts/rebirth-gateway-sim-validation.sh"
...
run_systemd_user_service_validation() {
  ...
  cleanup_systemd_user_service() {
    set +e
    (cd "$GATEWAY_BACKEND_DIR" && cargo run -q -p emwaver -- service uninstall >/dev/null 2>&1 || true)
    rm -rf "$state_dir"
    unset EMWAVER_STATE_DIR
  }
  trap cleanup_systemd_user_service RETURN

  (cd "$GATEWAY_BACKEND_DIR" && cargo run -q -p emwaver -- service install --port "$GATEWAY_SERVICE_PORT" --sim-device --now)
  ...
  node "$ROOT/scripts/verify-gateway-render.mjs" "$GATEWAY_SERVICE_PORT" emwaver-gateway
  (cd "$GATEWAY_BACKEND_DIR" && cargo run -q -p emwaver -- service uninstall)
}

echo
echo "== Linux systemd user service validation =="
(cd "$GATEWAY_BACKEND_DIR" && cargo run -q -p emwaver -- service print-unit --port "$GATEWAY_SIM_PORT" --sim-device >/tmp/emwaver-gateway.service)
grep -F "emwaver gateway serve" /tmp/emwaver-gateway.service >/dev/null
grep -F -- "--sim-device" /tmp/emwaver-gateway.service >/dev/null
grep -F -- "--port $GATEWAY_SIM_PORT" /tmp/emwaver-gateway.service >/dev/null
echo "ok: systemd user unit generation includes gateway serve, port, and simulator transport"
...
run_gateway_hardware_validation() {
  ...
  local args=(gateway serve --port "$GATEWAY_HARDWARE_PORT")
  ...
  (cd "$GATEWAY_BACKEND_DIR" && cargo run -q -p emwaver -- "${args[@]}" >"$gateway_log" 2>&1) &
  gateway_pid=$!
  ...
  echo
  node "$ROOT/scripts/verify-gateway-render.mjs" "$GATEWAY_HARDWARE_PORT" emwaver-gateway "$SCRIPT_PATH" "$GATEWAY_EVENT_TARGET_ID"
}

if [[ "$HARDWARE_TRANSPORT" == "ble" || -n "${EMWAVER_DEVICE_ID:-}" ]]; then
  echo
  echo "== Gateway hardware validation =="
...
== Gateway hardware validation skipped ==
...
3. Confirm EMWaver can scan:
   cd gateway/backend && cargo run -q -p emwaver -- devices
